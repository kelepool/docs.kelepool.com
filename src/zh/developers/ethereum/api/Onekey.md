...
这里请求的参数里有个`count`，需要传入用户本次需要生成的验证节点数量，传入的数量影响返回验证节点信息数量，这里传入`2`因此返回了两条验证者信息，你可能会发现他们的`提款凭证`都是一样的。由于用户可能在质押时传入了35、100、89这种并非32的倍数数量，因此Onekey需要计算一下有效质押数量，只允许用户质押32的倍数数量。
