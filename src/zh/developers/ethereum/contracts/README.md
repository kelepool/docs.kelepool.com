...
1.合约已经增加了V2版本的大额 、小额质押，支持质押时直接传递source渠道参数。

2.不会影响之前已经对接了V1版本的用户，可按需升级。

3.source渠道参数是可乐矿池分配给第三方渠道的标识，用于分红统计时区分是哪个渠道过来的质押金额。

4.如果已经接入了V1版本也没关系，更新成V2版本后，用户质押完成会优先写入从合约传递过来的source。
```
...
下边这个就是可乐矿池ETH质押合约的ABI数据，后边调用合约时都需要这个数据，本质上就是一个JSON字符串，这个数据在`Etherscan`上查看[逻辑合约](https://etherscan.io/address/0x89EA7948dF73aa35f0c4f772d334Be42c5ab8acF#code)时也能获得：
