...
1. The contract has added the V2 version of large and small pledges, and supports direct transmission of source channel parameters when pledging.

2. It will not affect users who have connected to the V1 version before, and can upgrade on demand.

3. The source channel parameter is the identifier assigned to the third-party channel by the Coke Mining Pool, which is used to distinguish which channel the pledged amount came from during the dividend statistics.

4. It doesn’t matter if you have already connected to the V1 version. After updating to the V2 version, the user’s pledge will be written to the source passed from the contract first.
```
...
let source = ethers.utils.hexlify(ethers.utils.toUtf8Bytes('tokenpocket'))
