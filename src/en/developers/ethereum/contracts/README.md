...
1. In order to improve the statistical accuracy of the channel pledge amount, the source channel parameter is supported when pledged through the contract.

2. This version will not affect third-party channels that have already connected to the earlier V1 version, and the V2 version can be upgraded on demand.

3. In the end, the system will give priority to using the source passed by the contract to calculate income and dividends for third-party channels.

4. For details, please refer to the code for calling the contract below.
```
